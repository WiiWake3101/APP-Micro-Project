...
class Admin_Employee_Salary_Table extends JPanel
{
    Admin_Employee_Salary_Table()
    {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery("SELECT * FROM salary_for_position");
            ...
            while (rs.next() && i < positionLabels.length) {
                ...
                double totalEmployees = rs.getDouble("HourlySalary");
                double currentEmployees = rs.getDouble("MonthlySalary");
                double requiredEmployees = rs.getDouble("OvertimePerHour");

                positionLabels[i].setText(positionName);
                totalLabels[i].setText(df.format(totalEmployees)); // Format the values with two decimal places
                currentLabels[i].setText(df.format(currentEmployees));
                requiredLabels[i].setText(df.format(requiredEmployees));
